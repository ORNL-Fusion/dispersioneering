...
if opts.use_root_finder
    ...
    for ii=1:num_points
        ...
        if ii==look_at_point
            ...
            for k = 1:num_init_kper2
                for j = 1:num_init_kper2
                    ...
                    [det(k,j),det_grad(k,j)] = det_fun(n_per_init,point_params);
        ...
        if ii==look_at_point
            ...
            dk = initial_k_pers(2)-initial_k_pers(1);
            [dudy,dudx] = gradient(real(det),dk); % need to add to this "h"
            [dvdy,dvdx] = gradient(imag(det),dk);
            det_grad2 = dudx+i*dvdx;
            det_grad3 = dvdy-i*dudy;
            figure
            contour(initial_k_pers2,initial_k_pers2,log10(abs(det_grad2)))
            colorbar
            figure
            contour(initial_k_pers2,initial_k_pers2,log10(abs(det_grad3)))
            colorbar
            figure
            contour(initial_k_pers2,initial_k_pers2,log10(abs(det_grad)))
            title('det_grad')
            colorbar
            figure
...
%             3.*c.*(exz + ezx).*kz.*n_per.^2.*w.^2 + 4.*exx.*n_per.^3.*w.^3))/c.^4);
        ...
        det_prime_re = 2.*exyre.*eyxim.*k0.^4.*kxim + 2.*exyim.*eyxre.*k0.^4.*kxim -  ...
            ...
            exzre.*eyyim.*k0.^4.*kzim - exzim.*eyyre.*k0.^4.*kzim + exyre.*eyzim.*k0.^4.*kzim +  ...
            exyim.*eyzre.*k0.^4.*kzim - eyyre.*ezxim.*k0.^4.*kzim - eyyim.*ezxre.*k0.^4.*kzim +  ...
            ...
            exzre.*eyyre.*k0.^4.*kzre + exyim.*eyzim.*k0.^4.*kzre - exyre.*eyzre.*k0.^4.*kzre -  ...
            eyyim.*ezxim.*k0.^4.*kzre + eyyre.*ezxre.*k0.^4.*kzre + eyxim.*ezyim.*k0.^4.*kzre -  ...
        ...
        det_prime_im = 2.*exyim.*eyxim.*k0.^4.*kxim - 2.*exyre.*eyxre.*k0.^4.*kxim -  ...
            2.*exxim.*eyyim.*k0.^4.*kxim + 2.*exxre.*eyyre.*k0.^4.*kxim +  ...
            2.*exzim.*ezxim.*k0.^4.*kxim - 2.*exzre.*ezxre.*k0.^4.*kxim -  ...
            2.*exxim.*ezzim.*k0.^4.*kxim + 2.*exxre.*ezzre.*k0.^4.*kxim +  ...
            4.*exxre.*k0.^2.*kxim.^3 - 2.*exyre.*eyxim.*k0.^4.*kxre -  ...
            2.*exyim.*eyxre.*k0.^4.*kxre + 2.*exxre.*eyyim.*k0.^4.*kxre +  ...
            2.*exxim.*eyyre.*k0.^4.*kxre - 2.*exzre.*ezxim.*k0.^4.*kxre -  ...
            2.*exzim.*ezxre.*k0.^4.*kxre + 2.*exxre.*ezzim.*k0.^4.*kxre +  ...
            2.*exxim.*ezzre.*k0.^4.*kxre + 12.*exxim.*k0.^2.*kxim.^2.*kxre -  ...
            12.*exxre.*k0.^2.*kxim.*kxre.^2 - 4.*exxim.*k0.^2.*kxre.^3 -  ...
            exzim.*eyyim.*k0.^4.*kzim + exzre.*eyyre.*k0.^4.*kzim + exyim.*eyzim.*k0.^4.*kzim -  ...
            exyre.*eyzre.*k0.^4.*kzim - eyyim.*ezxim.*k0.^4.*kzim + eyyre.*ezxre.*k0.^4.*kzim +  ...
            eyxim.*ezyim.*k0.^4.*kzim - eyxre.*ezyre.*k0.^4.*kzim +  ...
            3.*exzre.*k0.^2.*kxim.^2.*kzim + 3.*ezxre.*k0.^2.*kxim.^2.*kzim +  ...
            6.*exzim.*k0.^2.*kxim.*kxre.*kzim + 6.*ezxim.*k0.^2.*kxim.*kxre.*kzim -  ...
            3.*exzre.*k0.^2.*kxre.^2.*kzim - 3.*ezxre.*k0.^2.*kxre.^2.*kzim +  ...
            2.*exxre.*k0.^2.*kxim.*kzim.^2 + 2.*ezzre.*k0.^2.*kxim.*kzim.^2 +  ...
            2.*exxim.*k0.^2.*kxre.*kzim.^2 + 2.*ezzim.*k0.^2.*kxre.*kzim.^2 +  ...
            exzre.*k0.^2.*kzim.^3 + ezxre.*k0.^2.*kzim.^3 + exzre.*eyyim.*k0.^4.*kzre +  ...
            exzim.*eyyre.*k0.^4.*kzre - exyre.*eyzim.*k0.^4.*kzre - exyim.*eyzre.*k0.^4.*kzre +  ...
            eyyre.*ezxim.*k0.^4.*kzre + eyyim.*ezxre.*k0.^4.*kzre - eyxre.*ezyim.*k0.^4.*kzre -  ...
            eyxim.*ezyre.*k0.^4.*kzre + 3.*exzim.*k0.^2.*kxim.^2.*kzre +  ...
            3.*ezxim.*k0.^2.*kxim.^2.*kzre - 6.*exzre.*k0.^2.*kxim.*kxre.*kzre -  ...
            6.*ezxre.*k0.^2.*kxim.*kxre.*kzre - 3.*exzim.*k0.^2.*kxre.^2.*kzre -  ...
            3.*ezxim.*k0.^2.*kxre.^2.*kzre + 4.*exxim.*k0.^2.*kxim.*kzim.*kzre +  ...
            4.*ezzim.*k0.^2.*kxim.*kzim.*kzre - 4.*exxre.*k0.^2.*kxre.*kzim.*kzre -  ...
            4.*ezzre.*k0.^2.*kxre.*kzim.*kzre + 3.*exzim.*k0.^2.*kzim.^2.*kzre +  ...
            3.*ezxim.*k0.^2.*kzim.^2.*kzre - 2.*exxre.*k0.^2.*kxim.*kzre.^2 -  ...
            2.*ezzre.*k0.^2.*kxim.*kzre.^2 - 2.*exxim.*k0.^2.*kxre.*kzre.^2 -  ...
            2.*ezzim.*k0.^2.*kxre.*kzre.^2 - 3.*exzre.*k0.^2.*kzim.*kzre.^2 -  ...
            3.*ezxre.*k0.^2.*kzim.*kzre.^2 - exzim.*k0.^2.*kzre.^3 - ezxim.*k0.^2.*kzre.^3;
        
        det_gradient = det_prime_re + 1i*det_prime_im;
